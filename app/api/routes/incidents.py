...
#     # Track how many times each template has been used so we can assign
#     # state based on repetition number rather than global index.
#     # This ensures every template gets resolved instances (with datafix_code)
#     # regardless of where it falls in the global sequence.
#     template_repetition: dict = {}

#     for index in range(total_records):
#         template = INCIDENT_TEMPLATES[index % len(INCIDENT_TEMPLATES)]
#         template_key = index % len(INCIDENT_TEMPLATES)
#         rep = template_repetition.get(template_key, 0)
#         template_repetition[template_key] = rep + 1

#         # Assign state by per-template repetition so every template cycles
#         # through resolved states before becoming active.
#         state, active = STATES[rep % len(STATES)]
#         sequence = index + 1
...
INCIDENT_TEMPLATES = [
    ...
    },
    {
        ...
        "datafix_code": (
            ...
            "cur.execute('SELECT id, title, body, tags, updated_at FROM kb_articles')\n"
            "rows = cur.fetchall()\n"
            "for row in rows:\n"
            "    es.index(index='knowledge_articles', id=row[0], body={\n"
            "        'title': row[1], 'body': row[2],\n"
            "        'tags': row[3], 'updated': str(row[4])\n"
            "    })\n"
            "print(f'Re-indexed {len(rows)} articles.')\n"
            "conn.close()"
        ),
    },
    {
        "short_description": "SSL certificate expired on customer portal",
        "description": (
            "Customers cannot access the main portal. Browsers show ERR_CERT_DATE_INVALID. "
            "Certificate expired at midnight. Affects all external users. "
            "Internal staff using the admin bypass are unaffected."
        ),
        "category": "security", "subcategory": "certificate",
        "priority": "1 - Critical", "severity": "1 - Critical",
        "impact": "1 - High", "urgency": "1 - High",
        "resolution_notes": (
            "SSL certificate for portal.example.com had expired. Renewed certificate "
            "via Let's Encrypt, deployed to all load balancer nodes, and reloaded nginx. "
            "Added 30-day and 7-day expiry alerts to monitoring."
        ),
        "close_notes": "Certificate renewed. Monitoring alerts configured.",
        "group_key": "Security Operations",
        "datafix_code": (
            "#!/bin/bash\n"
            "# Datafix: renew Let's Encrypt cert and reload nginx\n"
            "# Incident: SSL certificate expired on customer portal\n\n"
            "DOMAIN='portal.example.com'\n"
            "echo \"Renewing certificate for $DOMAIN...\"\n"
            "certbot renew --cert-name \"$DOMAIN\" --force-renewal\n\n"
            "echo 'Deploying renewed cert to load balancer nodes...'\n"
            "for NODE in lb1 lb2 lb3; do\n"
            "  scp /etc/letsencrypt/live/$DOMAIN/fullchain.pem $NODE:/etc/nginx/ssl/\n"
            "  scp /etc/letsencrypt/live/$DOMAIN/privkey.pem   $NODE:/etc/nginx/ssl/\n"
            "  ssh $NODE 'sudo nginx -t && sudo systemctl reload nginx'\n"
            "done\n\n"
            "echo 'All nodes updated. Verifying...\n"
            "curl -I https://$DOMAIN | grep 'HTTP/'"
        ),
    },
    {
        "short_description": "Database backup job failing nightly",
        "description": (
            "Nightly database backup job has been failing for 3 days. "
            "DBA team received no alert emails. Backups are 3 days stale. "
            "No data loss yet but RPO is at risk."
        ),
        "category": "database", "subcategory": "backup",
        "priority": "2 - High", "severity": "2 - Major",
        "impact": "2 - Medium", "urgency": "2 - Medium",
        "resolution_notes": (
            "Backup job was writing to a mount point that had become read-only after "
            "a failed disk check. Remounted the backup volume read-write, cleared the "
            "stale lock file, and ran an immediate backup. Alert emails fixed by "
            "updating the SMTP relay hostname."
        ),
        "close_notes": "Backups running. Alerts working.",
        "group_key": "Database Administration",
        "datafix_code": (
            "#!/bin/bash\n"
            "# Datafix: remount backup volume and trigger immediate backup\n"
            "# Incident: Nightly DB backup failures\n\n"
            "BACKUP_MOUNT='/mnt/db-backups'\n"
            "LOCK_FILE='/var/run/db-backup.lock'\n\n"
            "echo 'Checking mount status...'\n"
            "mount | grep \"$BACKUP_MOUNT\"\n\n"
            "echo 'Remounting backup volume read-write...'\n"
            "sudo mount -o remount,rw \"$BACKUP_MOUNT\"\n\n"
            "echo 'Removing stale lock file...'\n"
            "sudo rm -f \"$LOCK_FILE\"\n\n"
            "echo 'Running immediate backup...'\n"
            "sudo -u postgres pg_dumpall | gzip > \"$BACKUP_MOUNT/backup_$(date +%Y%m%d_%H%M%S).sql.gz\"\n\n"
            "echo 'Verifying backup...'\n"
            "ls -lh \"$BACKUP_MOUNT/\" | tail -5"
        ),
    },
    {
        "short_description": "CI/CD pipeline failures after Docker base image update",
        "description": (
            "All CI/CD pipelines started failing with ImportError after the nightly "
            "base image was updated to python:3.12-slim. 14 services affected. "
            "New deployments blocked."
        ),
        "category": "devops", "subcategory": "ci_cd",
        "priority": "2 - High", "severity": "2 - Major",
        "impact": "2 - Medium", "urgency": "1 - High",
        "resolution_notes": (
            "Python 3.12 removed several deprecated stdlib modules used by "
...
def build_reference(table: str, sys_id: str, display_value: str = "") -> ReferenceField:
    """
    Build a ServiceNow-style ReferenceField.

    FIX: The `value` field is now set to `display_value` (the human-readable name)
    rather than `sys_id`.  Previously, `value` was always set to `sys_id`, so
    `_reference_value()` in the normaliser and ingestion pipeline returned an
    opaque UUID string instead of the group name.  This caused:
      1. The `assignment_group` stored in Qdrant to be a UUID like
         "287ebd7da9fe198100f92cc8d1d2154e" instead of "Network Operations".
      2. The frontend IncidentCard to display that UUID in the "Group" column.
      3. The LLM in step-1 of the agent to receive an unreadable group ID instead
         of a meaningful name, degrading analysis quality.

    When `display_value` is empty (e.g. for assigned_to / caller_id which don't
    have a name readily available) we fall back to `sys_id` to preserve the
    existing behaviour for those fields.
    """
    return ReferenceField(
        link=f"{SERVICENOW_INSTANCE}/api/now/table/{table}/{sys_id}",
        value=display_value if display_value else sys_id,
    )
...
def build_mock_incidents(total_records: int = 50) -> List[IncidentRecord]:
    ...
    for index in range(total_records):
        ...
        user_sys_id    = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"user-{sequence}"))
        caller_sys_id  = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"caller-{sequence}"))
        ci_sys_id      = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"ci-{sequence}"))
        ...
        incidents.append(
            IncidentRecord(
                ...
                # FIX: pass group_name as display_value so Qdrant stores the
                # human-readable name ("Network Operations") rather than the UUID.
                assignment_group=build_reference("sys_user_group", group_id, group_name),
                assigned_to=build_reference("sys_user", user_sys_id),
                caller_id=build_reference("sys_user", caller_sys_id),
                cmdb_ci=build_reference("cmdb_ci", ci_sys_id),
            )
        )
