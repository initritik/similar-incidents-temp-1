...
# - All similar resolved incidents (for resolution context).
# - A separate "INCIDENTS WITH DATAFIX CODE" section listing only the incidents
#   that have actual executable code in their datafix_code field.
 
# Your job:
# 1. Write a clear, actionable recommended_resolution (step-by-step, max 6 steps).
# 2. Look at the "INCIDENTS WITH DATAFIX CODE" section:
#    - If it is non-empty, you MUST produce a recommended_datafix by adapting
#      the most relevant datafix_code to the current incident. Do NOT return null.
#    - If it is empty (no incidents listed), set recommended_datafix to null.
...
#     # Send all incidents for resolution context, but ALSO send the datafix
#     # incidents as a clearly labelled separate section so the LLM cannot
#     # miss them or confuse empty datafix_code fields for "no datafix".
#     datafix_section = (
#         _serialize(datafix_incidents)
#         if datafix_incidents
#         else "[] — no incidents have datafix code; set recommended_datafix to null"
#     )
#     step2_user = (
#         f"User query:\n{user_query}\n\n"
#         f"Step 1 analysis:\n{_serialize(step1)}\n\n"
#         f"All similar incidents (resolution context):\n{_serialize(incidents)}\n\n"
#         f"INCIDENTS WITH DATAFIX CODE ({len(datafix_incidents)} found):\n{datafix_section}"
#     )
 
#     print(f"[agent] Step 2 — datafix_incidents count: {len(datafix_incidents)}")
#     print(f"[agent] Step 2 — datafix_section preview: {datafix_section[:300]}")
...
#     print(f"[agent] Step 2 raw LLM response: {step2_raw[:500]}")
 
#     _raw_datafix = step2.get("recommended_datafix")
#     # Normalize: LLMs sometimes return the string "null" instead of JSON null.
#     # Also treat empty / whitespace-only strings as no datafix.
#     if isinstance(_raw_datafix, str) and _raw_datafix.strip().lower() in ("null", "none", ""):
#         _raw_datafix = None
 
#     print(f"[agent] recommended_datafix after normalization: {repr(_raw_datafix)[:200] if _raw_datafix else None}")
...
#         recommended_datafix=_raw_datafix,
#     )
...
def _incidents_with_datafix(incidents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Return only the incidents whose datafix_code field is non-empty.

    FIX: Added explicit debug logging so it's easy to trace why this list
    might be empty if datafix_code is not being retrieved from Qdrant.
    The filter itself (`(i.get("datafix_code") or "").strip()`) correctly handles
    None, empty string, and whitespace-only values.
    """
    result = [i for i in incidents if (i.get("datafix_code") or "").strip()]
    print(
        f"[agent] _incidents_with_datafix: {len(incidents)} total, "
        f"{len(result)} with datafix_code. "
        f"datafix_code values: {[(i.get('number'), repr((i.get('datafix_code') or '')[:60])) for i in incidents]}"
    )
    return result
...
def _mock_agent(
    ...
    recommended_datafix: Optional[str] = None
    if datafix_incidents:
        source = datafix_incidents[0]
        emit(AgentEvent(
            "step_start",
            "Adapting datafix code",
            f"Source: {source.get('number')} — {source.get('short_description', '')[:60]}"
        ))
        # FIX: Added a defensive check to ensure datafix_code is actually a
        # non-empty string before building the recommended_datafix.  Previously
        # the code assumed datafix_incidents[0]["datafix_code"] was truthy because
        # _incidents_with_datafix filtered for it, but an extra strip() guard here
        # makes the intent explicit and prevents an empty recommended_datafix from
        # slipping through if the filter logic ever changes.
        raw_code = (source.get("datafix_code") or "").strip()
        if raw_code:
            recommended_datafix = (
                f"# Adapted from {source.get('number', 'N/A')}\n"
                f"# [Mock datafix — set ANTHROPIC_API_KEY or OPENAI_API_KEY for AI-generated code]\n\n"
                + raw_code
            )
            emit(AgentEvent("step_done", "Datafix code ready",
                            f"Adapted from {source.get('number', 'N/A')}"))
