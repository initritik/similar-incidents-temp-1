          ...
          for (const msg of messages) {
            ...
            try {
              ...
              if (eventType === "progress") {
                ...
                // Normalize recommended_datafix: the LLM sometimes returns the
                // string "null" instead of JSON null. Treat it as no datafix.
                const result = parsed as ChatResponse;
                if (
                  typeof result.recommended_datafix === "string" &&
                  result.recommended_datafix.trim().toLowerCase() === "null"
                ) {
                  result.recommended_datafix = null;
                }
                safeResolve(result);
