...
// ── AssistantContent: renders answer + agent log + incidents panel ────────────
//
// FIX: This component is defined at MODULE SCOPE (outside ChatPage), not inside
// the ChatPage render function.
//
// Bug that was present: AssistantContent was defined as a function component
// *inside* the ChatPage component body. React uses referential identity to track
// component types — every re-render of ChatPage produced a brand-new function
// reference for AssistantContent. React would see a different component type on
// each render, UNMOUNT the old tree and MOUNT a new one, which:
//   1. Reset IncidentsPanel's internal `activeTab` state to "incidents" on every
//      state change in ChatPage (including the isLoading → false transition that
//      happens right after a response arrives). This is exactly why the
//      "Recommended Datafix" tab kept disappearing / never stayed selected.
//   2. Caused unnecessary full re-mounts of the entire incidents panel tree.
//
// Moving it to module scope gives it a stable identity across renders, so React
// correctly reconciles it as the *same* component and preserves all internal state.
  ...
  const handleSendMessage = useCallback(
    async (payload: SendPayload) => {
      ...
      const onProgress = (event: AgentProgressEvent) => {
        ...
        // Update the placeholder assistant message with the latest events.
        // FIX: Because AssistantContent is now defined at module scope,
        // updating messages here no longer causes IncidentsPanel to remount.
        setMessages((prev) => {
          const updated = [...prev];
          updated[updated.length - 1] = {
            role: "assistant",
      ...
      try {
        ...
        // Replace placeholder with the full response (events still shown).
        // FIX: AssistantContent is now module-scoped, so IncidentsPanel's
        // internal activeTab state is NOT reset by this setState call.
        const assistantMessage: ChatMessageProps = {
          role: "assistant",
          content: (
            <AssistantContent
